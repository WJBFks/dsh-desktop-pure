...
- **Built-in "桌面端配置" page (DSH Desktop Pure page)**: the "页面" button left of `File` in the title bar opens a dropdown to switch **桌面端配置 / DSH Web**. The Pure page mirrors the DSH Web settings-page style (four sections: Appearance / DSH Web / Changelog / About), supports light / dark, and offers a full-window / card layout toggle (persisted). It is **fully independent of dsh web** — it works even when the server is down. The app starts on DSH Web; if it can't connect, it falls back to the Pure page instead of exiting.
