...
- **内置 DSH Desktop Pure 页**：标题栏「文件」左侧的鲸鱼图标可下拉切换 **DSH Desktop Pure / DSH Web**。纯页参考 DSH Web 设置页风格（外观 / 连接 / 关于），支持浅色 / 深色，**完全独立于 dsh web**——服务未启动也能用；启动默认进入 DSH Web，连不上则自动回退到纯页，不退出应用。
