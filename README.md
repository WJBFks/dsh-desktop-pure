...
- **内置「桌面端配置」页（DSH Desktop Pure 页）**：标题栏「文件」左侧的「页面」按钮可下拉切换 **桌面端配置 / DSH Web**。纯页参考 DSH Web 设置页风格（外观 / DSH Web / 更新日志 / 关于 四个分区），支持浅色 / 深色，并提供全屏 / 卡片布局切换（持久化）；DSH Web 分区支持**多端点 Tab**——自动判断当前系统（Windows 本机）、自动检测并拉起 WSL 内的 dsh web、可添加自定义远程地址（持久化）；**完全独立于 dsh web**——服务未启动也能用；启动默认进入 DSH Web，连不上则自动回退到纯页，不退出应用。
