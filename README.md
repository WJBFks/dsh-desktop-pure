...
- **零侵入**：不修改 DSH 代码 / 资源 / 配置，只作为独立桌面外壳运行。
- **多端点**：支持 Windows 本机、WSL、自定义远程地址，切换端点不丢失页面会话。
- **WSL 引导**：WSL / Node / DSH 未就绪时提供四步引导页，可自动安装并连接。
- **原生体验**：自绘标题栏、菜单、托盘、主题切换，保持桌面端使用习惯。
...
| **纯 WebUI 套壳** | 界面默认由 `dsh web` 提供；壳只绘制**自己的**标题栏 / 下拉菜单 / 加载页 / **内置 DSH Desktop Pure 页** / 托盘（几个壳自有的静态 HTML），从不注入或修改 Harness 的 DOM |
| **不修改源程序** | Harness 页面、`window.__DSH_BOOT__` 注入、`/api/*` RPC 全部原样来自 `dsh web` 进程，壳只负责把它加载进来并加桌面化外壳 |

## 功能

- **端口策略（单端口，绝不静默漂移）**：复用优先（端口已有 Harness → 直接复用，状态标注「已复用」）；端口空闲 → 自动拉起 `dsh web`；被其他进程占用 → 弹窗给出**进程名 + PID**，「重试 / 改用 DSH Desktop Pure / 退出」。
- **自绘单行标题栏**：状态点（🔵 Pure 页 / 🟢 已连接 / 🟡 启动中 / 🔴 已断开）+ `文件 / 视图 / 服务器` 菜单（固定在按钮正下方弹出、打开时 hover 即切换）+「在浏览器中打开」+ 窗口控制（Win/Linux 自绘，macOS 原生交通灯）；窗口标题固定为 *DSH Desktop Pure*。
- **内置「桌面端配置」页（DSH Desktop Pure 页）**：标题栏「文件」左侧的「页面」按钮可下拉切换 **桌面端配置 / DSH Web**。纯页参考 DSH Web 设置页风格（外观 / DSH Web / 更新日志 / 关于 四个分区），支持浅色 / 深色，并提供全屏 / 卡片布局切换（持久化）；DSH Web 分区支持**多端点 Tab**——自动判断当前系统（Windows 本机）、自动检测并拉起 WSL 内的 dsh web、可添加自定义远程地址（持久化）；WSL 未就绪时提供四步引导页（安装 WSL / 安装 Node / 安装 DSH / 连接），支持自动安装与自动启动；**完全独立于 dsh web**——服务未启动也能用；启动默认打开 Windows 页面，连不上则停留在端点自己的路由层状态页，不退出应用。
...
- **安全加固**：渲染进程 `sandbox` + `contextIsolation` + 无 `nodeIntegration`；导航白名单为 loopback + 已注册端点主机（WSL IP / 远程地址）；其它外链交系统浏览器；禁用 `<webview>`。
...
3. **远程 DSH Web 增强** —— 自定义远程地址已支持（v0.3.0，只读连接）；后续可评估远端启动 / 重启等管理能力。
