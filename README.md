...
- **内置「桌面端配置」页（DSH Desktop Pure 页）**：标题栏「文件」左侧的「页面」按钮可下拉切换 **桌面端配置 / DSH Web**。纯页参考 DSH Web 设置页风格（外观 / DSH Web / 更新日志 / 关于 四个分区），支持浅色 / 深色，并提供全屏 / 卡片布局切换（持久化）；**完全独立于 dsh web**——服务未启动也能用；启动默认进入 DSH Web，连不上则自动回退到纯页，不退出应用。
